...
<html>
  ...
  <body onload="exportForm.howMany.focus();">
    ...
    <h1>Export Phone Numbers for territory number: <?php
    ...
    ?><form action="" method="get"><select name="territory_id"><?php
      ...
      ((address_export_id IS NULL OR address_export_id = 0)
        ...
        AND (status_id NOT IN(3,5) or status_id IS NULL))
                  AND (
                    (
                      number_of_tries < 3
                      OR number_of_tries IS NULL
                    )
                    AND phone_number IS NOT NULL
                    AND phone_number <> '')
                  AND territory_id = territory_queue.territory_id)";
      $res=$con->query("
      SELECT distinct territory_queue.territory_id, territory_queue.order_number, territories.territory_number territory_number,
      ...
      while ($row = $res->fetch_assoc()) {
        ...
        echo '<option value="'.$territory_id.'">Territory #'.$territory_number.' ('.$available_exports.' Available Phone Numbers)</option>';
      } ?>
    ...
    ?></h1>
    <p>Select how many phone numbers you want to export.</p>
    Available to export:
      <?php
        ...
        if($howMany > 0) {
          ...
          switch($_POST['fileType']) {
            ...
            while ($row = $res->fetch_assoc()) {
              ...
              if($addressList == "") {
                $addressList = $row["name"] . "," . $row["phone_number"] . "," . $row["address"] . $daySleeper . "\r\n";
              }
              else {
                $addressList = $addressList . $row["name"] . "," . $row["phone_number"] . "," . $row["address"] . $daySleeper . "\r\n";
              }
              global $territory_number;
              $territory_number = $row['territory_number'];
            }
      ...
      ?>
    <form action="export_phone_numbers.php?territory_id=<?php echo $territory_id; ?>" name="exportForm" method="POST" onsubmit="return howManyTest();">
      ...
      <?php } ?>
        <tr>
          <td><label for="howMany">How Many Phone Numbers to Export?:</label></td>
